...
DEBUG_HASH=$(echo $DEBUG_STRING | sha1sum | awk '{print $1}')

BASE_STORAGE_FOLDER="/storage/jenkins-lava/baremetal-tests/debug-open-enoent"
...
echo "DEBUG_STRING=$DEBUG_STRING" >> properties.txt
...
echo "STORAGE_KERNEL_IMAGE=$BASE_STORAGE_FOLDER/kernel/$KERNEL_VERSION-$KERNEL_COMMIT_ID.$BUILD_DEVICE.$DEBUG_HASH.bzImage" >> properties.txt
echo "STORAGE_LINUX_MODULES=$BASE_STORAGE_FOLDER/modules/linux/$KERNEL_VERSION-$KERNEL_COMMIT_ID.$BUILD_DEVICE.$DEBUG_HASH.linux.modules.tar.gz" >> properties.txt
echo "STORAGE_LTTNG_MODULES=$BASE_STORAGE_FOLDER/modules/lttng/$KERNEL_VERSION-$KERNEL_COMMIT_ID-$LTTNG_MODULES_COMMIT_ID.$BUILD_DEVICE.$DEBUG_HASH.lttng.modules.tar.gz" >> properties.txt
